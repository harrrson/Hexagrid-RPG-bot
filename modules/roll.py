...
async def _roll(dice_size: int = 10, n_of_rolls: int = 1, roll_modifier: int = 0,
...
class Rolling(commands.Cog, name='Dice rolling'):
    ...
    async def roll(self, ctx, rolls: typing.Optional[int] = 1, roll_command: str = 'd10', *, args=''):
        ...
        for i in range(rolls):
            roll_list, roll_result = await _roll(dice_size=dice_size, n_of_rolls=simult_rolls,
                                                 roll_modifier=modifier, roll_modifier_type=operator)
    ...
    async def fate(self, ctx, *, args: str = ''):
        ...
        if '!' in args:
            comment = ' Comment: ' + args[args.find('!') + 1:]
        else:
            comment = ''
        if result:
