...
async def _split_roll_command(command: str):
    ...
    if command.count('d') == 1 and (plus_count + minus_count + mul_count +
                                    div_count) < 2 and not re.search(
                                        '[a-ce-z!@#$%^&(){}[]:~`";=_,.?/|]',
                                        command):
...
async def _roll(dice_size: int = 10,
                n_of_rolls: int = 1,
                roll_modifier: int = 0,
                roll_modifier_type: str = '+'):
    result = 0
...
class Rolling(commands.Cog, name='Dice rolling'):
    __max_rolls = 30
    __max_dice = 100
    __dice_colours = ('green', 'lime', 'yellow', 'white', 'orange', 'red',
                      'black')
    __colour_rolls = {
    ...
    __duel_texts = [
        "Let's get ready to RUMBLEEEEE!", "It's duel time!",
    ...
    @commands.group(invoke_without_command=True,
                    help='Roll dices depending of the parameters given')
    async def roll(self,
                   ctx,
                   rolls: typing.Optional[int] = 1,
                   roll_command: str = 'd10',
                   *,
                   args=''):
        message = ['']
        roll_results = []
        if rolls > self.__max_rolls or rolls < 1:
            ...
            return
        valid_command, simult_rolls, dice_size, modifier, operator = await _split_roll_command(
            roll_command)
        ...
        for i in range(rolls):
            roll_list, roll_result = await _roll(dice_size=dice_size,
                                                 n_of_rolls=simult_rolls,
            ...
            if len(message[message_fragments_count] + '\n' +
                   roll_message) <= 2000:
                message[message_fragments_count] = message[
                    message_fragments_count] + '\n' + roll_message
            else:
        ...
        if rolls == 1:
            if len(message[message_fragments_count] + comment) <= 2000:
                message[message_fragments_count] = message[
                    message_fragments_count] + comment
            else:
                message_fragments_count += 1
    ...
    @roll.command(name='duel',
                  help='Makes a duel between two players, and shows a winner')
    async def duel(self,
                   ctx,
                   roll1='d10',
                   roll2='d10',
                   ...
                   *,
                   args=''):
        valid_command1, simult_rolls1, dice_size1, modifier1, operator1 = await _split_roll_command(
            roll1)
        ...
        if not valid_command1 or not valid_command2:
            await ctx.send('Command is not valid.')
            return
        if (operator1 == '/' and modifier1 == 0) or (operator2 == '/'
                                                     and modifier2 == 0):
            await ctx.send('You cannot divide by 0.')
            return
        ...
        if dice_size1 < 2 or dice_size1 > self.__max_dice or dice_size2 < 2 or dice_size2 > self.__max_dice:
            ctx.send(f'You need to choose dice between 2-{self.__max_dice}')
        roll_list1, result1 = await _roll(dice_size1, simult_rolls1, modifier1,
                                          operator1)
        roll_list2, result2 = await _roll(dice_size2, simult_rolls2, modifier2,
                                          operator2)
        message = random.choice(self.__duel_texts) + '\n'
        if player1 and player2:
            message += "   " + player1 + " vs " + player2 + "\n"
        if result1 > result2:
            message += "   " + ("  " * len(player1)) + "__**" + str(
                result1) + "**__      " + str(result2) + "\n"
            if player1:
