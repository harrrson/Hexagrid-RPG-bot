...
async def _split_roll_command(command: str):
    ...
    if command.count('d') == 1 and (
            plus_count + minus_count + mul_count + div_count) < 2 and not re.search('[a-ce-z!@#$%^&(){}[]:~`";=_,.?/|]',
                                                                                    command):
...
class Rolling(commands.Cog, name='Dice rolling'):
    ...
    async def roll(self, ctx, rolls: typing.Optional[int] = 1, roll_command: str = 'd10', explosion: str = '', *,
                   args=''):
        """
